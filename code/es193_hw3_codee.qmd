...
e. Test implications\newline
I found that as temperature increases, the kelp elongation rate decreases. Based on these results we can infer that kelp growth rates will be lowered during warmer weather which may occur more frequently due to climate change. A decrease in kelp growth rate would be detrimental for kelp forest ecosystems and all the animals that depend on them. To conserve kelp forest ecosystems, we should mitigate climate change, which is definitely a tall task, or plant out kelp subspecies that are more heat tolerant. 
...
Both the Pearson's correlation test and Spearman's correlation test would have led me to make the same decision. Both tests found a moderate negative relationship between temperature and kelp frond elongation rate, and the Pearson's r was about equal to the Spearman's $\rho$ (Pearson's correlation, r = -0.6877489, t(30) = -5.189, p < 0.0001, $\alpha$ = 0.05; Spearman's correlation, $\rho$ =  -0.6891684, S = 9216.1, p < 0.0001, $\alpha$ = 0.05). 
...
```
**Figure 2. Computer Screen Time in Relation to Non-Computer Activity Time.** The filled in purple points represent a given days Non-Computer Activity Time and Computer Screen Time. Data last updated 2026-05-28.
...
Overall, the authors handle "visual clutter" pretty good as they have taken efforts to increase their data to ink ratio such as removing the figure background and gridlines. The authors do not include the underlying data through a jitterplot over the boxplot which I think was a good choice since there are ten boxplots which would have meant a lot of points crowding each smaller boxplot. 

d. Recommendations\newline

One thing I could change is making the text slighly bigger to increase reading accesibility. As it is, it is hard to read some of the numbers until you open the "figure viewer" on the webpage, and even when you do the font size is a little too small and could be increased. 
